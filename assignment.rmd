...
## The Model 
Since this is a classification problem I will use tree-based-models. A single tree can easily be interpreted, but the covariates itself are not very interpretable (a lot of sensor-data), so interpretably  is not so much of an issue here. A random forest is much more powerful.  
Building a random forest based on conditional inference trees (cforest) did not work (computer ran out of memory). So a *cforest* is not an option (for me). 
So I will go for a **random forest**. 
  
## Results Random Forest

The trainingset will be used for building the model. 
I used the validationset for cross-validation. Let's take a look at the confusionmatrix: 
 
```{r rf,echo=FALSE,cache=TRUE,message=FALSE}
